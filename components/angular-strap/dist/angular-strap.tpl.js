...
angular.module('mgcrea.ngStrap.typeahead').run([
  '$templateCache',
  function ($templateCache) {
    $templateCache.put('typeahead/typeahead.tpl.html', '<ul class="typeahead no__bullet" ng-show="$isVisible()" role="select"><li role="presentation" ng-repeat="match in $matches" ><a role="menuitem" ng-click="$select($index, $event)" ng-bind="match.label"></a></li></ul>');
  }
]);
